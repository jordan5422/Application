...
if (!empty($_POST)) {
    ...
    if (!empty($_FILES['photo']['name'])) {
        ...
        if ($uploadOk == 1) {
            $sql = "UPDATE ingredients SET nom = ?, origine = ?, quantite = ?, photo = ?, description = ? WHERE id = ?";
            $stmt = $mysqlClient->prepare($sql);
    
            try {
                $stmt->execute([$name, $origine, $quantite, $target_file, $description, $donnee['id']]);
  ...
  <body>
    ...
    <main class="page">
     <section class="contact-container">
          <article>
            <form class="form contact-form" action="ModificationING.php" method="post" enctype="multipart/form-data">
              <div class="form-row">
              ...
              <div class="form-row">
                ...
                <input type="text" name="origine" id="origine" class="form-input" value="<?php echo $donnee['origine'] ?>" required />
              </div>
              <div class="form-row">
                <label for="name" class="form-label">Quantité</label>
                <input type="text" name="quantite" id="quantite" class="form-input" value="<?php echo $donnee['quantite'] ?>" required />
              </div>
              <div class="form-row">
              ...
              <div class="form-row">
                ...
                <textarea name="message" id="message" class="form-textarea"  required><?php echo $donnee['description'] ?></textarea>
              </div>
